...
@pytest.fixture
def list_url(comment):
    return reverse(
        "api:v1:comment_attachment_list",
        kwargs={"comment_id": comment.pk}
    )


@pytest.fixture
def detail_url(comment_attachment):
    return reverse(
        "api:v1:comment_attachment_detail",
        kwargs={
            "comment_id": comment_attachment.comment_id,
            "pk": comment_attachment.pk
        }
    )
...
class TestCommentAttachmentListCreateAPIView:

    def test_pagination_works(self, admin_client, list_url, comment,
                              comment_attachment_factory, http_host):
        comment_attachment_factory.create_batch(5, comment=comment)
    ...
    def test_perform_create(self, admin_client, list_url, comment,
...
@pytest.mark.django_db
class TestCommentAttachmentRetrieveDestroyAPIView:
